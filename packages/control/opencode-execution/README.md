...
- **No lifecycle events beyond the A1 vocabulary.** `execution/commanded` settlement is DERIVED (the fold marks an execution settled when its commanded stream ends); there is no `execution/settled` event in the vocabulary.
