...
- **`EffectExecutor`** — kernel-gated effect safety on real DSH backing: structural validation → typed capability authorize → canonical `effect/requested` + `effect/authorized`/`effect/denied` → derived outbox working view → scoped worker with retries (each retry appends a distinct `effect/attempt-started`) → canonical outcome. `commit-unknown` breaks the loop (never blind-retry IRREVERSIBLE); a dedicated `reconcile()` resolves any AMBIGUOUS action — an explicit `commit-unknown` or an orphaned `attempt-started` (a worker killed between dispatch and outcome) — with a monotonic `${action_id}:reconcile:${n}` attempt id derived from the canonical log and `reconcile: true`. A second `execute()` for a resolved or ambiguous action is rejected (`ERR_EFFECT_REENTRY`).
...
| [`src/effect-executor.ts`](src/effect-executor.ts) | Kernel-gated effect dispatch, retries, ambiguous (`commit-unknown` / orphaned-attempt) → `reconcile()`, monotonic reconcile attempt ids, `ERR_EFFECT_REENTRY` guard |
...
- **Real external-effect reconciliation protocols are deferred (P4+).** `reconcile()` resolves an ambiguous action (a prior `commit-unknown`, or an orphaned `attempt-started` whose worker died between dispatch and outcome) through the worker with `reconcile: true` and a monotonic reconcile attempt id; production reconciliation protocols (external-state inspection, idempotency keys, saga compensation) are out of scope here.
